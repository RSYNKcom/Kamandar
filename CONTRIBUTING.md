...
cd releaser
ruby -c sprint_plate.rb   # syntax check
ruby test_plate.rb        # run the acceptance tests
```
...
`test_plate.rb` is the spec of record. It uses a fixed "today"
...
- Run `ruby test_plate.rb` before opening a PR; it must print `0 failed`.
...
3. Keep the in-file README block (the header of `sprint_plate.rb`) in sync with
...
- Confirm `ruby test_plate.rb` passes.
